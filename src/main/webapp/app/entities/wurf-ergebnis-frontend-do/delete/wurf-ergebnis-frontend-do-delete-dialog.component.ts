import { Component, inject } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { NgbActiveModal } from '@ng-bootstrap/ng-bootstrap';

import SharedModule from 'app/shared/shared.module';
import { ITEM_DELETED_EVENT } from 'app/config/navigation.constants';
import { IWurfErgebnisFrontendDo } from '../wurf-ergebnis-frontend-do.model';
import { WurfErgebnisFrontendDoService } from '../service/wurf-ergebnis-frontend-do.service';

@Component({
  templateUrl: './wurf-ergebnis-frontend-do-delete-dialog.component.html',
  imports: [SharedModule, FormsModule],
})
export class WurfErgebnisFrontendDoDeleteDialogComponent {
  wurfErgebnis?: IWurfErgebnisFrontendDo;

  protected wurfErgebnisService = inject(WurfErgebnisFrontendDoService);
  protected activeModal = inject(NgbActiveModal);

  cancel(): void {
    this.activeModal.dismiss();
  }

  confirmDelete(id: number): void {
    this.wurfErgebnisService.delete(id).subscribe(() => {
      this.activeModal.close(ITEM_DELETED_EVENT);
    });
  }
}
